...
class SimpleNet(torch.nn.Module):
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt):
        ...
        np.save("output/binplot_data/image_scores.npy", np.array(scores))
        np.save("output/binplot_data/image_labels.npy", np.array(labels_gt))
        self.save_segmentation_images(test_data, segmentations, scores)
        auroc = metrics.compute_imagewise_retrieval_metrics(
            scores, labels_gt 
        )["auroc"]
    ...
    def train(self, training_data, test_data, use_checkpoint=False):
